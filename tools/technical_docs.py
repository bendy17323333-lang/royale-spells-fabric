...
CHAPTERS = (
    ...
    "10-extension-and-troubleshooting.md", *GENERATED,
)
...
def source_index() -> str:
    ...
    groups = [(".", "公共入口与机制"), ("iron", "铁魔法接入"), ("army", "军团"),
...
def spell_reference() -> str:
    ...
    for path in recipes:
        recipe = data(path)
        require(recipe["type"] == "minecraft:crafting_shapeless", f"New recipe type; update renderer: {path}")
        ingredients = Counter()
        ...
        require(not conditions or conditions == absent_iron, f"New recipe condition; update renderer: {path}")
        ...
        recipe_rows.append([link(path, path.stem), " + ".join(f"{count}×`{item}`" for item, count in ingredients.items()),
                            f"{result.get('count',1)}×`{result['id']}`", "未加载铁魔法" if conditions else "无模组条件"])
    lines += [f"## 数据包无序合成配方（{len(recipes)}）", "",
