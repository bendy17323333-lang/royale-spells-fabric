...
public final class TroopModel<T extends LivingEntity> extends EntityModel<T> implements ModelWithArms {
    ...
    public TroopModel(String kind) {
        this.kind=kind;
        try(var reader=MinecraftClient.getInstance().getResourceManager().openAsReader(RoyaleSpells.id("models/troop/"+kind+".json"))) {
    ...
    @Override public void setAngles(T entity,float limbAngle,float limbDistance,float age,float headYaw,float headPitch) {
        parts.values().forEach(p->{p.pitch=0;p.yaw=0;p.roll=0;});
        float walk=MathHelper.cos(limbAngle*.65f)*limbDistance;float breathe=MathHelper.sin(age*.07f)*.025f;
        pose("head",headPitch*MathHelper.RADIANS_PER_DEGREE,headYaw*MathHelper.RADIANS_PER_DEGREE,0);
        pose("right_leg",walk,0,0);pose("left_leg",-walk,0,0);
        pose("right_arm",-walk*.7f+breathe,0,.04f);pose("left_arm",walk*.7f-breathe,0,-.04f);
        pose("cape",.12f+Math.abs(walk)*.18f,MathHelper.sin(age*.055f)*.035f,0);
        pose("banner",0,MathHelper.sin(age*.07f)*.08f,MathHelper.sin(age*.09f)*.04f);
        pose("jaw",Math.abs(walk)*.12f,0,0);
        if(handSwingProgress>0){float strike=MathHelper.sin(handSwingProgress*MathHelper.PI);pose("right_arm",-strike*1.9f-.2f,0,-strike*.12f);pose("body",0,-strike*.1f,0);}
    ...
    private void transform(Part p,MatrixStack m,float unit){
        m.translate(p.pivot[0]*unit,p.pivot[1]*unit,p.pivot[2]*unit);
        m.multiply(RotationAxis.POSITIVE_Z.rotation(p.rotation[2]*MathHelper.RADIANS_PER_DEGREE+p.roll));
    ...
    private void renderPart(Part p,MatrixStack m,VertexConsumer v,int light,int overlay,float r,float g,float b,float a) {
        m.push();transform(p,m,1);
