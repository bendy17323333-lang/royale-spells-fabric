...
[Summoned](../../src/main/java/dev/royalespells/entity/Summoned.java) 是本模组召唤物的公共接口，提供主人、剩余生命周期初始化和克隆标志。当前实现类为 `AllyZombie`、`AllySkeleton`、`RoyaleUnit`、`ElementalSpirit`，军团继承 `AllySkeleton`。
...
军团使用单独 `doHurtTarget` 和动画节奏。Beta 1 因这个重写遗漏了墓园的穿受伤间隔逻辑；本地 Beta 2 在军团小兵自己的伤害调用中补上同样的局部清除/恢复，正常与虚化小兵都生效，将军保持普通受伤间隔。并为虚化小兵增加保留方块碰撞的友方分散，详见 [第 11 章](11-furnace-staff-and-spirits.md)。
...
将军死亡或以会销毁实体的原因移除时，结束对应账本；区块卸载本身不等于军团死亡。只有幽灵发现账本失效后停止追击并在 20 tick 后消散。未虚化小兵保持当前血量和归属继续近战、不会再虚化，剩余寿命不延长。
...
公开 Beta 2 的 [ArmyCombatGoal](../../src/main/java/dev/royalespells/army/ArmyCombatGoal.java) 让小兵更积极地冲锋、将军稍慢跟随并在贴身时迎战，不产生后退逃跑锚点，小兵按稳定存档槽位分配接敌方向，堵塞时沿侧翼接近。近处使用经过地面和方块碰撞检查的小数坐标转向，远处沿用原版寻路；幽灵转化会清理旧起手、路径与死亡计时。具体常量、分散规则和实测边界见 [11.8 军团修复](11-furnace-staff-and-spirits.md#8-骷髅军团修复)。
