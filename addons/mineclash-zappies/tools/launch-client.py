...
repo=work/'mineclash-zappies-addon'
...
assert mode in ('baseline','pack','fixed','combat','battle') and re.fullmatch(r'[a-zA-Z0-9_-]*',suffix)
...
if mode=='pack' or 'pack' in suffix:
    for p in Path('C:/Users/BliBe/curseforge/minecraft/Instances/1.21.1/mods').glob('*.jar'):
        if mode in ('fixed','combat','battle') and p.name.startswith('royale-spells-'):continue
        shutil.copy2(p,run/'mods'/p.name)
    if mode in ('fixed','combat','battle'):
...
values={'natives_directory':str(run/'natives'),'launcher_name':'ZappiesIsolatedQA','launcher_version':'1',
 ...
 'resolution_width':'1280' if mode=='battle' else '1600','resolution_height':'800' if mode=='battle' else '1000'}
def args(seq):
    result=[]
    for a in seq:
    ...
    return result
params=['-Xmx3G','-Dzappiesaddon.smoke=true']
